(function(){
  "use strict";
  var KEY="fokusyou-v05";
  var OLD_KEYS=["fokusyou-v04","fokusyou-v03"];
  ...
  var defaults={
    xp:35,successes:3,focus:{title:"Heute bewusst vorankommen",note:"Wähle eine Aufgabe, die heute besonders wichtig ist."},
    completedGoals:0,
    routines:[
  ...
  function migrate(raw){
    ...
    raw.focus=raw.focus||clone(defaults.focus);raw.completedGoals=raw.completedGoals||0;raw.events=raw.events||[];raw.work=raw.work||clone(defaults.work);
  ...
  function render(){
    ...
    document.getElementById("profileAchievements").textContent=state.successes;document.getElementById("profileStreak").textContent=best;
    renderFeaturedGoal();renderRoutines();renderGoals();renderEvents();renderWork();renderCalendarDate();save()
  }
  ...
  document.addEventListener("click",function(e){
    ...
    var r=e.target.closest("[data-routine]");if(r){var routine=state.routines.find(function(x){return x.id===r.dataset.routine});if(routine){routine.done=!routine.done;if(routine.done){routine.streak++;state.xp+=5;state.successes++}else{routine.streak=Math.max(0,routine.streak-1);state.xp=Math.max(0,state.xp-5)}render()}return}
  ...
  function changeGoal(id,direction){
    ...
    if(direction>0&&g.value>old){state.xp+=2;state.successes++;(g.milestones||[]).forEach(function(m){if(g.value>=m&&!g.rewarded.includes(m)){g.rewarded.push(m);state.xp+=10}});if(old<g.target&&g.value>=g.target){state.completedGoals=(state.completedGoals||0)+1;state.xp+=25;alert("🏆 Ziel erreicht! 25 Bonus-XP.")}}
